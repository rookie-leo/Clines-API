...
public class FlightFactory {
	...
	public FlightFactory(WaypointFormMapper waypointFormMapper, ExistingAircraftService aircraftService,
			ExistingAirportService airportService) {
		this.waypointFormMapper = waypointFormMapper;
	...
	private void populateAircraft(FlightForm form, ArrayList<Exception> violations) {
		aircraftService.findByCode(form.getAircraftId()).ifPresentOrElse(form::setAircraft,
	...
	private void populateWaypointForm(WaypointForm form, ArrayList<Exception> violations, String message) {
		airportService.findById(form.getAirportId()).ifPresentOrElse(form::setAirport,
				() -> violations.add(new AirportNotFoundException(message)));
	}
}
